...
final class BridgeUserNotifier {
    ...
    // 原来是 "com.brycewg.asrkb.imebridge"——那是 BiBi Bridge 单独作为一个 App 安装时
    // 的包名。合并进 GboardHooker 之后它不再是一个独立安装的 App，
    // createPackageContext() 找不到这个包会直接抛异常，所有提示 Toast 会静默失效。
    // 这里必须换成合并后真实的 applicationId。
    private static final String MODULE_PACKAGE = "io.github.a110789.gboardhooker";
